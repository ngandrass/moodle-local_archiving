...
final class process_uploaded_artifact_test extends \advanced_testcase {
     ...
     *
     * @covers \archivingmod_assign\external\process_uploaded_artifact
     *
     * @return void
     * @throws \coding_exception
     * @throws \dml_exception
    ...
    /**
     * Tests that files from the draft area of other users are rejected and not touched
     *
     * @covers \archivingmod_assign\external\process_uploaded_artifact
     *
     * @return void
    ...
    public function test_rejection_of_foreign_draft_area(): void {
        // Create task for admin user and a draft file that belongs to another user.
        $this->resetAfterTest();
        $this->setAdminUser();
        $mocks = $this->getDataGenerator()->create_mock_task(wstoken: 'TEST-WS-TOKEN');
    ...
    public function test_rejection_of_wrong_task_type(): void {
        ...
        $DB->update_record(
            ...
            ['id' => $mocks->task->get_id(), 'archivingmod' => 'quiz']
        );
