...
class archivingtrigger extends \core\plugininfo\base {
    ...
    public function is_uninstall_allowed(): bool {
        if ($this->name === 'manual') {
            // The manual archiving trigger is required by the core and should not be uninstalled separately.
            return false;
        }
    ...
    public static function enable_plugin(string $pluginname, int $enabled): bool {
        // Allow to pass the plugin name with or without the 'archivingtrigger_' prefix.
        if (str_starts_with($pluginname, 'archivingtrigger_')) {
            $pluginname = substr($pluginname, strlen('archivingtrigger_'));
        }

        // Determine if the plugin was enabled before.
        $wasenabled = (get_config("archivingtrigger_{$pluginname}", 'enabled') == 1);

        // Enable or disable the plugin.
        if ($enabled && !$wasenabled) {
            set_config('enabled', 1, "archivingtrigger_{$pluginname}");
            return true;
        } else if (!$enabled && $wasenabled) {
            set_config('enabled', 0, "archivingtrigger_{$pluginname}");
            return true;
        }
    ...
    public static function get_enabled_plugins() {
        ...
        foreach (plugin_util::get_archiving_triggers() as $name => $metadata) {
            if ($metadata['enabled']) {
