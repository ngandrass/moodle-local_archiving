...
final class archivingtrigger_test extends \advanced_testcase {
     ...
     *
     * @covers \local_archiving\plugininfo\archivingtrigger
     *
     * @return void
     */
    public function test_flags(): void {
        ...
        // Validate flags.
        if ($plugin->name === 'manual') {
            $this->assertFalse($plugin->is_uninstall_allowed(), "{$plugin->component} subplugin must not allow uninstallation.");
        } else {
        ...
        // Ensure that the manual archiving trigger is always enabled.
        $manualtrigger = \core_plugin_manager::instance()->get_plugin_info('archivingtrigger_manual');
        if (!$manualtrigger) {
            $this->markTestSkipped('The manual archiving trigger subplugin is not installed.');
        }
        $this->assertFalse($manualtrigger->is_uninstall_allowed(), "The manual archiving trigger must not allow uninstallation.");
    }
    ...
    public function test_report_enabled_plugins(): void {
        ...
        $this->assertSameSize(
            $enabledplugins,
            archivingtrigger::get_enabled_plugins(),
